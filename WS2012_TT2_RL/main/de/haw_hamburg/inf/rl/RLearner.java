...
public class RLearner {
    ...
    public static final int Q_LAMBDA   = 3;           // Good parms were
                                                       // lambda=0.05,
        ...
        switch (learningMethod) {
            ...
            case Q_LAMBDA: {
                ...
                while (!thisWorld.endState()) {
                    ...
                    System.arraycopy(state, 0, saPair, 0,
                            state.length);
                    ...
                    for (int e = trace.size() - 2; e >= 0; e--) {
                        ...
                        System.arraycopy(saPair, 0, state, 0,
                                state.length);
                        ...
                        new_Q = this_Q
                                + alpha
                                * delta
                                * Math.pow(gamma * lambda, (trace
                                        .size() - 1 - e));

                        policy.setQValue(state, action, new_Q);

                        // System.out.println("Set Q:" + new_Q + "for " +
                        // state[0] + "," + state[1] + " action " + action );
                    }
        ...
        switch (actionSelection) {
            ...
            case SOFTMAX: {
                ...
                while (!valid) {
                    ...
                    for (action = 0; action < qValues.length; action++) {
                        if (rndValue > offset
                                && rndValue < offset + prob[action])
                            selectedAction = action;
                        offset += prob[action];
                        // System.out.println( "Action " + action +
                        // " chosen with " + prob[action] );
                    }
