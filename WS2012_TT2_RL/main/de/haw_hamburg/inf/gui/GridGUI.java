...
public class GridGUI implements Observer {
    ...
    private void initialize() {
        ...
        txtTargethoptime.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                gridc.setTargetHopTime(Integer
                        .parseInt(txtTargethoptime.getText()));
            }
        });
    ...
    public void update(Observable o, Object arg) {
        if (o instanceof GridControl) {
            ...
            System.out.println("TRYING TO TOGGLE PANEL!!");
            togglePanel((int) arg);
        } else {
            moveAgent((int[]) arg);
    ...
    private void moveAgent(int[] arg) {
        ...
        for (int i = 0; i < 4; i++) {
            ((JLabel) grid[agentPos[0]][agentPos[1]].getComponents()[i])
                    .setText(Double.toString(agent
                            .getQValues(agentPos)[i]));
            if (agent.getQValues(agentPos)[i] > maxQ) {
                maxQ = agent.getQValues(agentPos)[i];
                ((JLabel) grid[agentPos[0]][agentPos[1]]
                        .getComponents()[i]).setForeground(Color.RED);
                last = ((JLabel) grid[agentPos[0]][agentPos[1]]
                        .getComponents()[i]);
            } else {
                ((JLabel) grid[agentPos[0]][agentPos[1]]
                        .getComponents()[i])
