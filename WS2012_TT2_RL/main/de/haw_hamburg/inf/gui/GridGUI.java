...
public class GridGUI implements Observer {
	...
	private void initialize() {
		...
		gridPanel.setLayout(new GridLayout(dimension[1], dimension[0], 0, 0));
		...
		for (y = 0; y < dimension[1]; y++) {
			for (x = 0; x < dimension[0]; x++) {
				grid[x][y] = new JPanel();
				setGridAction(grid[x][y]);
				grid[x][y].setLayout(null);
				grid[x][y].setForeground(Color.BLACK);
				grid[x][y].setBorder(new LineBorder(new Color(0, 0, 0)));
		...
		btnStart.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (btnStart.isSelected()) {
					...
					agent.setExplorationRate(Integer.parseInt(txtExploration
							.getText()));
					gc.setEpisodes(Integer.parseInt(txtEpisodes.getText()));
		...
		JRadioButton rdbtnFixedTarget = new JRadioButton("fixed target");
		...
		JRadioButton rdbtnCirclingTarget = new JRadioButton("circling target");
		...
		JLabel lblNumberOfSteps = new JLabel("Number of steps in best Episode:");
		...
		btnResetLearning.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (y = 0; y < dimension[1]; y++) {
					for (x = 0; x < dimension[0]; x++) {
						for (i = 0; i < 4; i++) {
							((JLabel) grid[x][y].getComponents()[i])
									.setText("0");
							((JLabel) grid[x][y].getComponents()[i])
	...
	private void setGridAction(final JPanel jPanel) {
	...
	private void setUpTargetLabel(final JLabel lblTarget) {
		lblTarget.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getButton() == MouseEvent.BUTTON1) {
					gc.terminate();
					togglePanel(Integer.parseInt(lblTarget.getParent()
							.getName()));
				}
			}
		});
	...
	public void togglePanel(int panelNr) {
		...
		grid[9][panelNr].getComponent(4).setEnabled(
				grid[9][panelNr].getComponent(4).isEnabled() ? false : true);
	}
	...
	private void moveAgent(int[] arg) {
		...
		for (int i = 0; i < 4; i++) {
			((JLabel) grid[agentPos[0]][agentPos[1]].getComponents()[i])
					.setText(Double.toString(agent.getQValues(agentPos)[i]));
			if (agent.getQValues(agentPos)[i] > maxQ) {
				maxQ = agent.getQValues(agentPos)[i];
				((JLabel) grid[agentPos[0]][agentPos[1]].getComponents()[i])
						.setForeground(Color.RED);
				last = ((JLabel) grid[agentPos[0]][agentPos[1]].getComponents()[i]);
			} else {
				((JLabel) grid[agentPos[0]][agentPos[1]].getComponents()[i])
						.setForeground(Color.BLACK);
				if (!last.equals(null))
